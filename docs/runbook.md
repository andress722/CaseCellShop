...
| `dlq_depth` >0 ou `dlq_messages_total` sobe | Pedidos requerem inspeção; correlacionar jobId/orderId, `erp_errors_total` e último erro.                          | Executar `npm run reconcile` após verificar o ERP.              | Reenfileirar apenas quando idempotência estiver confirmada; validar fatura única e status terminal. |
...
4. Rodar `npm run reconcile`. A rotina consulta pedidos incertos, envelhecidos e a DLQ.
5. Verificar estado final, reserva do inventário e exatamente uma linha em `erp_invoices` por pedido.
